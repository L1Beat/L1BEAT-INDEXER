...
import { Menu, X, ChevronDown, ChevronRight, BarChart3, Activity, ArrowLeftRight, ExternalLink, FileText, TrendingUp, Users, Fuel } from 'lucide-react'
...
const menuGroups: MenuGroup[] = [
    {
        ...
        items: [
            ...
            { label: 'TPS Stats', path: '/tps' },
            { label: 'Cumulative Txs', path: '/cumulative-txs' },
            { label: 'Chain Comparison', path: '/chain-comparison', icon: <TrendingUp size={16} /> }
        ]
    },
    {
        ...
        icon: <Users size={18} />,
        items: [
            { label: 'Daily Active Users', path: '/daily-active-addresses' },
            { label: 'User Periods', path: '/user-analytics' }
        ]
    },
    {
        ...
        icon: <Fuel size={18} />,
        items: [
            { label: 'Gas Usage', path: '/gas-usage' },
            { label: 'Gas Analytics', path: '/gas-analytics' }
        ]
    },
    {
        title: 'Cross-Chain',
        ...
        items: [
            ...
            { label: 'ICM vs LayerZero', path: '/messaging-comparison' },
            { label: 'ICM Messages', path: '/icm-messages' },
...
export default function Sidebar({ isMobileMenuOpen, setIsMobileMenuOpen }: SidebarProps) {
    ...
    const MenuContent = () => (
        <nav className="h-full overflow-y-auto">
            <div className="p-4">
                <h1 className="text-xl font-bold mb-2 text-blue-600">L1BEAT INDEXER</h1>
                <p className="text-xs text-gray-500 mb-6">Avalanche L1 Analytics</p>
                                    ...
                                    {group.items.map(item => (
                                        item.external ? (
                                            ...
                                            <Link
                                                ...
                                                className={`flex items-center justify-between px-2 py-1.5 rounded text-sm transition-colors ${isActive(item.path)
